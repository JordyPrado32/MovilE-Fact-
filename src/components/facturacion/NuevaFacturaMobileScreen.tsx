    ...
    <>
      ...
      {saving ? (
        <View style={styles.directoryLoading}>
          ...
          <Text style={styles.mutedText}>Tu factura está siendo enviada al SRI...</Text>
        </View>
      ) : null}
      {loading ? (
        <View style={styles.directoryLoading}>
      ...
      {step === 0 ? <>
        ...
        {cliente ? <View style={[styles.formSectionBox, styles.invoicePanel]}>
          ...
          <Field label="Correo adicional (solo esta factura)" value={form.correoAdicional} onChangeText={(value) => onChange('correoAdicional', value)} autoCapitalize="none" keyboardType="email-address" />
        </View> : null}
      ...
      {step === 2 ? <>
        ...
        <View style={styles.formActions}><SecondaryButton label="Volver a productos" onPress={() => setStep(1)} /><SecondaryButton label="Limpiar" onPress={handleClear} /><PrimaryButton label="Generar factura" loading={saving} onPress={onSave} /></View>
      </> : null}
    </>
  );
}
