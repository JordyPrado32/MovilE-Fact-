...
export function MenuItem({ accentColor, active, disabled, expanded, hasChildren, inset, label, onPress, onToggle }: { accentColor?: string; active: boolean; disabled?: boolean; expanded?: boolean; hasChildren?: boolean; inset?: boolean; label: string; onPress: () => void; onToggle?: () => void }) {
  return <Pressable accessibilityLabel={label} accessibilityState={{ disabled: Boolean(disabled), expanded: hasChildren ? Boolean(expanded) : undefined, selected: active }} disabled={disabled} style={[styles.menuItem, inset && styles.menuItemInset, active && styles.menuItemActive, active && accentColor && { backgroundColor: accentColor, borderColor: accentColor }, disabled && styles.menuItemDisabled]} onPress={onPress}><Text style={[styles.menuItemText, active && styles.menuItemTextActive]} numberOfLines={1}>{label}</Text>{hasChildren ? <Pressable accessibilityLabel={`${expanded ? 'Contraer' : 'Expandir'} ${label}`} accessibilityRole="button" hitSlop={10} onPress={(event) => { event.stopPropagation(); onToggle?.(); }}><Text style={[styles.menuItemChevron, expanded && styles.menuItemChevronExpanded, active && styles.menuItemTextActive]}>⌄</Text></Pressable> : null}</Pressable>;
}
