...
export function SearchField({ label, value, onChangeText, placeholder, resultCount, totalCount, loading, onSubmit }: {
  label: string;
  value: string;
  onChangeText: (value: string) => void;
  placeholder?: string;
...
export function LoginActionTiles({ active, biometricLabel, onPassword, onBiometric, onGuest }: { active: 'password' | 'biometric' | 'guest'; biometricLabel: string | null; onPassword: () => void; onBiometric: () => void; onGuest: () => void }) {
  return <View style={styles.loginActionTiles}><LoginActionTile active={active === 'password'} icon="account-outline" label="Usuario y\ncontraseña" onPress={onPassword} /><LoginActionTile active={active === 'biometric'} icon="fingerprint" label={biometricLabel ? biometricLabel : 'Huella /\nFace ID'} onPress={onBiometric} /><LoginActionTile active={active === 'guest'} icon="dots-horizontal-circle-outline" label="Invitado" onPress={onGuest} /></View>;
}
...
export function SegmentButton({ active, label, onPress }: { active: boolean; label: string; onPress: () => void }) {
  return <Pressable accessibilityLabel={label} accessibilityState={{ selected: active }} style={[styles.segmentButton, active && styles.segmentButtonActive]} onPress={onPress}><Text style={[styles.segmentText, active && styles.segmentTextActive]}>{label}</Text></Pressable>;
}
